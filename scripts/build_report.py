...
CONTENT = [
    ...
    ("table", ("Таблица В.1 — Соответствие практических работ компонентам системы",
               ...
               [
                   ...
                   ("Практическая работа 3", "Обработка данных: слоистая архитектура, модель витрины, дедупликация, справочники.", "Предстоит"),
    ...
    ("p", "19 сентября 2026 года загрузчики были проверены на реальных ответах обоих источников по 18 сентября включительно. Для этого использована временная SQLite-база с теми же уникальными ключами; результаты первого и повторного запуска приведены в Таблице 2.3."),
    ...
    ("p", "Ограничением этапа является то, что контрольный прогон выполнен во временной базе SQLite, а работа с PostgreSQL в контейнере пока не подтверждена. Сохранение исходных фрагментов JSON и XML в слое raw создаёт основу для последующей прослеживаемости происхождения данных — профильного компонента направления «Управление данными»."),
